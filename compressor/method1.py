...
class Method1Compressor(CompressorBase):
    ...
    def compress(self, arr: List[int]) -> None:
        ...
        for num in overflow_list:
            bit_string += num
        arr.clear()
        arr.append(arr_len)  # Store the length of the original array
    ...
    def decompress(self, arr: List[int]) -> None:
        ...
        nb_of_int = arr.pop(0)                       # on recupere la taille du tableau
        ...
        bit_string = bit_string[12:]                    # Remove the first 18 bits used for metadata
        arr_len = len(arr)
        cpt = 0
    ...
    def get(self, compressed_data: List[int], index: int) -> int:
        ...
        """
        arr_len = compressed_data[0]
        bit_string = get_bin(compressed_data[1], SIZE_OF_INT)   # on recupere le premier entier
        max_bits = int(bit_string[:6], 2)           # on recupere la taille des entiers normaux
        big_max_bits = int(bit_string[6:12], 2)     # on recupere la taille des entiers en overflow 
        # arr_len = int(bit_string[12:18], 2)         # on recupere la taille du tableau
        indice_elem = index * (max_bits + 1) + 12 + SIZE_OF_INT       # on calcule l'indice du i-eme element dans la chaine de bits
        ...
        overflow_start = arr_len * (max_bits + 1) + 12 + SIZE_OF_INT # 18 pour les 3x6 bits du début
